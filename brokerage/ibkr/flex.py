...
def _ensure_event_loop_for_ib_async_import() -> None:
    """Create a thread-local event loop before importing ib_async."""
...
def _load_ibkr_exchange_mappings() -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"IBKR exchange_mappings.yaml not found at {path}") from exc
    except Exception as exc:
        raise RuntimeError(f"Failed to load IBKR exchange mappings from {path}: {exc}") from exc
    if not isinstance(payload, dict) or not payload:
        raise RuntimeError(f"IBKR exchange mappings must be a non-empty mapping at {path}")
    return payload
...
_IBKR_INSTRUMENT_TYPE_BY_CODE: dict[str, InstrumentType] = {
    "STK": "equity",
    "OPT": "option",
    "FOP": "option",
    "FUT": "futures",
...
def _map_instrument_type(asset_category: Any) -> InstrumentType:
    """Map one exact IBKR instrument code to a canonical instrument kind."""
    category = str(asset_category or "").strip().upper()
    if not category:
        raise ValueError("IBKR Flex trade missing instrument type claim")
    try:
        return _IBKR_INSTRUMENT_TYPE_BY_CODE[category]
    except KeyError as exc:
        raise ValueError(
            f"unsupported IBKR Flex instrument type claim: {category!r}"
        ) from exc


def _resolve_trade_instrument_type(trade: Any) -> tuple[str, InstrumentType]:
    """Resolve agreeing authoritative IBKR type fields for one Flex trade."""
    claims: list[tuple[str, str, InstrumentType]] = []
    for field in ("assetCategory", "assetClass", "security_type", "secType"):
        raw_value = _get_attr(trade, field)
        value = str(raw_value or "").strip().upper()
        if value:
            claims.append((field, value, _map_instrument_type(value)))

    if not claims:
        raise ValueError("IBKR Flex trade missing instrument type claim")

    distinct_kinds = {instrument_type for _, _, instrument_type in claims}
    if len(distinct_kinds) != 1:
        rendered = ", ".join(
            f"{field}={value}" for field, value, _ in claims
        )
        raise ValueError(f"conflicting IBKR Flex instrument type claims: {rendered}")

    _, category, instrument_type = claims[0]
    return category, instrument_type
...
def normalize_flex_trades(flex_trades: Iterable[Any]) -> List[Dict[str, Any]]:
    ...
    - Unmappable side/date rows are skipped with a warning.
    ...
    for trade in flex_trades:
        ...
        asset_category, instrument_type = _resolve_trade_instrument_type(trade)
        ...
        currency = _normalize_flex_currency(_get_attr(trade, "currency"))
...
def normalize_flex_prior_positions(
    ...
    for row in rows:
        ...
        currency = _normalize_flex_currency(row_dict.get("currency"))
...
def _normalize_flex_currency(value: Any) -> str:
    ...
    if len(text) == 3 and text.isalpha():
        return text
    raise ValueError("IBKR Flex row requires an explicit ISO currency code")
...
def _check_flex_error_response_xml(root: Any) -> tuple[int | None, str | None]:
    ...
    try:
        ...
        logger.warning("IBKR Flex response contains a non-integer ErrorCode")
        return None, None
...
def _download_flex_report(
    ...
    *,
    ...
    try:
        ...
        return None, "IBKR Flex Phase 1 request failed"
    ...
    if status_el is None or (status_el.text or "").strip() != "Success":
        ...
        return None, f"IBKR Flex Phase 1 error {code}"
    ...
    while elapsed < poll_timeout:
        ...
        try:
            ...
            return None, "IBKR Flex Phase 2 poll failed"
...
def _write_report_xml(raw_xml: str | None, save_xml_path: str | None) -> None:
    ...
    try:
        ...
        logger.warning(
            "Failed to save IBKR Flex XML exception_type=%s",
            type(exc).__name__,
        )
...
def _load_flex_report(
    *,
    ...
    if path:
        ...
        if error_code is not None:
            return None, f"IBKR Flex error {error_code} in {path}"
