...
from ._vendor import safe_float as _safe_float
...
def _build_contract(symbol: str, sec_type: str, exchange: str, currency: str):
    ...
    if len(curr) != 3 or not curr.isalpha():
        raise IBKRContractError("Contract currency requires an explicit ISO code")
...
def _normalize_contract_detail(contract_detail) -> dict[str, Any]:
    ...
    if isinstance(valid_exchanges_raw, str):
        valid_exchanges = [
            e.strip() for e in valid_exchanges_raw.split(",") if e.strip()
        ]
...
def _normalize_chain(chain) -> dict[str, Any]:
    ...
    strikes = sorted(
        [
            strike
            for strike in (_safe_float(v) for v in strikes_raw)
            if strike is not None
        ]
    )
    expirations = sorted(
        [str(v) for v in list(getattr(chain, "expirations", []) or [])]
    )
    return {
...
def fetch_option_chain(
    ...
    *,
    ...
    curr = str(currency or "").strip().upper()
    if len(curr) != 3 or not curr.isalpha():
        raise IBKRContractError("Underlying currency requires an explicit ISO code")

    if sec == "STK":
        underlying = Stock(sym, exchange, curr)
        fut_fop_exchange = ""
    elif sec == "FUT":
        ...
        raise IBKRContractError(
            f"Option chain supports STK or FUT underlyings, got {sec}"
        )
    ...
    if sec == "FUT":
        fut_fop_exchange = str(
            getattr(resolved, "exchange", "") or fut_fop_exchange or exchange
        )
