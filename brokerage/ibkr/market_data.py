...
def _historical_error_detail(error_code: Any, error_string: Any) -> str | None:
    ...
    # Historical bars usually report request failures via errorEvent while
    # reqHistoricalData itself simply returns an empty list.
    historical_codes = {
        162,  # Historical Market Data Service error
        165,  # Historical data query message
        166,  # Expired contract violation
    ...
    lowered = text.lower()
    if (
        code in historical_codes
        or "historical market data" in lowered
        or "eodchart" in lowered
    ):
        return f"{code_text}: {text}" if code_text else text
    return None


def _unique_error_details(messages: list[str]) -> str:
...
class IBKRMarketDataClient:
    ...
    def _coerce_snapshot_contract(self, contract: Any):
        """Accept typed boundary specs or raw vendor contracts."""
        if isinstance(contract, IBKRContractSpec):
            return self._resolve_spec(contract)

        if isinstance(contract, dict) or getattr(contract, "_ibkr_contract_spec", False):
            raise IBKRContractError(
                "Legacy snapshot contract specs are no longer supported; "
                "pass brokerage.ibkr.IBKRContractSpec instead"
            )
        ...
        with _ibkr_request_lock:
            ...
            try:
                ...
                def _on_error(reqId: int, errorCode: int, errorString: str, *_extra: Any) -> None:
                    del reqId, _extra
                    detail = _historical_error_detail(errorCode, errorString)
                    if detail:
                        historical_errors.append(detail)
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                if not bars:
                    detail = _unique_error_details(historical_errors)
                    if detail:
                        lowered = detail.lower()
                        if (
                            "entitlement" in lowered
                            or "market data permission" in lowered
                            or "permission" in lowered
                            or "not subscribed" in lowered
                        ):
                            raise IBKREntitlementError(detail)
                        if "no security definition" in lowered or "unknown contract" in lowered:
                            raise IBKRContractError(detail)
                        raise IBKRNoDataError(f"No historical bars returned: {detail}")
    ...
    def fetch_series(
        ...
        *,
        ...
        _last_no_data: IBKRNoDataError | None = None
        for candidate in chain:
            try:
                request_kwargs = {
        ...
        if raise_on_no_data and _last_no_data is not None:
            raise _last_no_data
        return pd.Series(dtype=float)
    ...
    def fetch_monthly_close_option(
        ...
        *,
        ...
        series = self.fetch_series(
            ...
            profile=profile,
            contract_identity=contract_identity,
            raise_on_transient=raise_on_failure,
            raise_on_no_data=raise_on_failure,
            budget_user_id=budget_user_id,
        )
        if series.empty:
            return series
