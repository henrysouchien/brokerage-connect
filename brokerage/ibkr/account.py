...
def _portfolio_frame_from_items(
    ...
    *,
        ...
        rows.append(
            {
                ...
                # The updatePortfolio stream populates primaryExchange (the listing
                # venue) and never exchange, which stays "". Carrying only the latter
                # dropped the venue for every portfolio row.
    ...
    return frame.sort_values(["account", "symbol"], na_position="last").reset_index(
        drop=True
    )
...
def _wait_for_pnl_ready(
    ib, pnl_obj, *, timeout_seconds: float, poll_interval: float
) -> None:
